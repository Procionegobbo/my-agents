...
2. **Determine numbering.** Check existing files in both `STORIES/TODO/` and `STORIES/COMPLETED/` and continue from the highest numeric prefix found (3-digit zero-padded, e.g. `042-`). Numbering is global across all features and must never collide.

3. **Write one file per story** in `STORIES/TODO/`, named `NNN-short-kebab-name.md`, numbered in implementation order (dependencies first).
...
**Dependencies:** [story numbers/files this depends on, or None]
```
...
- Dependencies may only point to lower-numbered stories.
...
- [ ] Numbering continues correctly from the highest prefix in `STORIES/TODO/` and `STORIES/COMPLETED/`.
- [ ] Every story has the Spec reference, Gherkin criteria, Technical Notes, Tests, Priority, and Dependencies.
- [ ] Dependencies only reference lower-numbered stories.
